...
const MSG_TEN_SDT = "- Tên phải có ít nhất 7 ký tự chữ cái.<br />- Số điện thoại phải đủ 10 số.<br/>- Số điện thoại phải bắt đầu bằng số '0'!!!";
...
function xemDsDatSan(day) {
	...
	resetTables();
	$.ajax({
		url: "/CN/soccer/api/xemdatsan.php",
		type: "GET",
		cache: false,
		data: {
			action: "xemdatsan",
...
function veTableDatSan(data) {
	...
	html += "<thead><tr><th>#</th><th>Tên KH</th><th>SĐT</th><th>Sân</th><th>Bắt đầu</th><th>Kết thúc</th><th>Phút</th><th>Đơn giá (/phút)</th><th>Tiền</th><th>Đã thanh toán</th><th class='thanhtoan'>Thanh toán</th><th class='xoa'>Xoá</th></tr></thead>";
	var tong_tien = 0;
	var da_thanh_toan = 0;
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (thanh_toan == "0") {
			html += "<td class='xoatdtt'><button class='btnThanhToan' datsan_id='" + data[i].datsan_id + "'>Thanh toán</button></td>";
		} else {
			html += "<td class='xoatdtt'><button disabled class='btnThanhToan' datsan_id='" + data[i].datsan_id + "'>Thanh toán</button></td>";
		}
		
		html += "<td class='xoatddatsan'><button class='btnXoaDatSan' datsan_id='" + data[i].datsan_id + "'>Xóa</button></td>";
		html += "</tr>";
	}
	html += "<tr class='a'><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>Đã thanh toán</b></td><td style='font-weight:bold;color:green;'>" + formatMoney(da_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr class='a'><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>Chưa thanh toán</b></td><td style='font-weight:bold;color:red;'>" + formatMoney(chua_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr class='a'><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>Tổng tiền</b></td><td style='font-weight:bold;color:blue;'>" + formatMoney(tong_tien) + "đ</td><td></td><td></td><td></td></tr>";
	html += "</table>";
	$("#ds_datsan").html(html);
...
function xoaDatSan(datsan_id) {
	$.ajax({
		url: "/CN/soccer/api/xoadatsan.php",
		type: "POST",
		cache: false,
		data: {
			datsan_id : datsan_id
...
function thanhToanDatSan(datsan_id) {
	$.ajax({
		url: "/CN/soccer/api/thanhtoandatsan.php",
		type: "POST",
		cache: false,
		data: {
			datsan_id: datsan_id
